...
export function startLive(sessionId: string, openf1SessionKey: string = "latest", pollIntervalS?: number) {
    return request("/admin/live/start", {
        method: "POST",
        body: JSON.stringify({
            session_id: sessionId,
            openf1_session_key: openf1SessionKey,
            poll_interval_s: pollIntervalS ?? null,
        }),
    });
}

export function stopLive(sessionId: string) {
...
import type { RaceBrief, PitRejoinResult, UndercutResult, RecommendStrategyResult } from "./types";
...
export function recommendStrategy(sessionId: string, driverCode: string) {
    return request<RecommendStrategyResult>("/tools/recommend_strategy", {
        method: "POST",
        body: JSON.stringify({ session_id: sessionId, driver_code: driverCode }),
    });
}
